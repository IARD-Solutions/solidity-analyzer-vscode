...
contract ChildContract is VulnerableBase {
    ...
    function checkUserCredentials(string memory username, string memory password) public pure returns (bool) {
        // Unsafe hash comparison
        return keccak256(abi.encodePacked(username, password)) == keccak256(abi.encodePacked("admin", "secret"));
    }
